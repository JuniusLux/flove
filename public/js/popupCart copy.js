...
const CartManager = {
  ...
  updatePopupUI(item) {
    ...
    if (!productId || !productType || !addButton || !counter || !counterNumber) {
  ...
  addToCart(item) {
    ...
    this.cart[productType][productId] = (this.cart[productType][productId] || 0) + 1;
  ...
  changeQuantity(item, isIncrease) {
    ...
    if (isIncrease) {
      ...
      this.cart[productType][productId] = Math.max((this.cart[productType][productId] || 1) - 1, 0);
...
document.querySelectorAll(".item-card__name, .item-card__details, .item-card__image").forEach((card) => {
  card.addEventListener("click", function () {
    ...
    if (item) {
      ...
      CartManager.updatePopupUI(document.querySelector(`.item[data-id="${item.id}"][data-type="${itemType}"]`));
    } else {
